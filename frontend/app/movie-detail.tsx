...
export default function MovieDetailScreen() {
  ...
  const [voteAverage, setVoteAverage] = useState(
    parseFloat(params.voteAverage || '0')
  );
  const [overview, setOverview] = useState(params.overview || '');

  const [providers, setProviders] = useState<StreamingProvider[]>(() => {
    try {
      return params.providersJson ? JSON.parse(params.providersJson) : [];
    } catch {
      return [];
    }
  });
  const [cast, setCast] = useState<CastMember[]>(() => {
    try {
      return params.creditsJson ? JSON.parse(params.creditsJson) : [];
    } catch {
      return [];
    }
  });
  const [crewMembers, setCrewMembers] = useState<CrewMemberData[]>(() => {
    try {
      return params.crewJson ? JSON.parse(params.crewJson) : [];
    } catch {
      return [];
    }
  });
  const [detailedInfo, setDetailedInfo] = useState<DetailedInfoData>(() => {
    try {
      return params.detailedInfoJson ? JSON.parse(params.detailedInfoJson) : {};
    } catch {
      return {};
    }
  });
  ...
  // Rating system
  const { rating: savedRating, isWatched: savedIsWatched } =
    useMovieRating(tmdbId);
      ...
      backendAPIService
        ...
        .then(res => {
          if (res.success && res.data) {
            ...
            if (providers.length === 0 && d.providers?.length > 0)
              setProviders(d.providers);
            if (cast.length === 0 && d.credits?.length > 0) setCast(d.credits);
            if (crewMembers.length === 0 && d.crew?.length > 0)
              setCrewMembers(d.crew);
            if (
              !detailedInfo?.genres?.length &&
              d.detailedInfo &&
              Object.keys(d.detailedInfo).length > 0
            ) {
              ...
              {isAuthenticated && (
                ...
                >
                  <Ionicons
                    ...
                    size={20}
                    color={
                      isWatched ? '#22c55e' : isDark ? '#a3a3a3' : '#737373'
                    }
                  />
                  <Text
