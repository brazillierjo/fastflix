  ...
  return (
    <SafeAreaView className='flex-1 bg-light-background dark:bg-dark-background'>
      ...
      >
        ...
        {/* Recommended for you - guest mode with setup data */}
        {!isAuthenticated && hasSetupData && guestRecommendations.length > 0 && (
          <MotiView
            from={{ opacity: 0, translateY: 15 }}
            animate={{ opacity: 1, translateY: 0 }}
            transition={{ delay: 400, type: 'timing', duration: 600 }}
          ...
          >
            ...
            <ScrollView
              ...
              contentContainerStyle={{ paddingHorizontal: 24, gap: 12 }}
            >
              {guestRecommendations.map((item: { tmdb_id?: number; id?: number; poster_path?: string }, i: number) => (
                <MotiView
                  key={item.tmdb_id || item.id || i}
                  from={{ opacity: 0, scale: 0.9 }}
                  animate={{ opacity: 1, scale: 1 }}
        ...
        >
          ...
          <View
            style={[getSquircle(12), getCardShadow(isDark)]}
            className='flex-row items-center gap-4 border border-light-border bg-light-surface px-4 py-5 dark:border-dark-border dark:bg-dark-surface'
          >
            <Ionicons
            ...
            <View className='flex-1'>
              <Text className='text-sm text-light-muted dark:text-dark-muted'>
                {t('home.continueExploringHint') ||
                  'Pick up where you left off and discover new recommendations.'}
              </Text>
            </View>
            <TouchableOpacity
              onPress={() => router.push('/search' as never)}
              style={getSquircle(12)}
              className='bg-netflix-500 px-4 py-2'
            >
              <Text className='text-sm font-semibold text-white'>
                {t('home.explore') || t('tabs.search')}
              </Text>
            </TouchableOpacity>
          </View>
        </MotiView>
