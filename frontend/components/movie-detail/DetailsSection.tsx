    ...
    <>
      ...
      {hasAnyDetail && (
        ...
        >
          ...
          >
            {detailedInfo.original_title &&
              detailedInfo.original_title !== title && (
                <View className='mb-3 flex-row items-center gap-2'>
                  ...
                  <Text className='flex-1 text-sm text-light-textSecondary dark:text-dark-textSecondary'>
                    {t('movieDetail.originalTitle') || 'Original title'}:{' '}
                    {detailedInfo.original_title}
                  </Text>
                </View>
              )}
            {detailedInfo.original_language &&
              detailedInfo.original_language !== langCode?.slice(0, 2) && (
                <View className='mb-3 flex-row items-center gap-2'>
                  <Ionicons
                  ...
                  />
                  <Text className='flex-1 text-sm text-light-textSecondary dark:text-dark-textSecondary'>
                    {t('movieDetail.originalLanguage') || 'Original'}:{' '}
                    {getLanguageName(detailedInfo.original_language)}
                  </Text>
                </View>
              )}
            {detailedInfo.spoken_languages &&
              detailedInfo.spoken_languages.length > 0 && (
                <View className='mb-3 flex-row items-center gap-2'>
                  <Ionicons
                  ...
                  />
                  <Text className='flex-1 text-sm text-light-textSecondary dark:text-dark-textSecondary'>
                    {t('movieDetail.spokenLanguages') || 'Languages'}:{' '}
                    {detailedInfo.spoken_languages
            ...
            {detailedInfo.production_countries &&
              detailedInfo.production_countries.length > 0 && (
                <View className='mb-3 flex-row items-center gap-2'>
                  <Ionicons
                    name='globe-outline'
                    size={15}
            ...
            {mediaType === 'movie' &&
              ...
              detailedInfo.budget > 0 && (
                <View className='mb-3 flex-row items-center gap-2'>
                  ...
                  />
                  <Text className='flex-1 text-sm text-light-textSecondary dark:text-dark-textSecondary'>
                    {formatCurrency(detailedInfo.budget)}{' '}
                    {t('movieDetail.budget') || 'budget'}
            ...
            {detailedInfo.belongs_to_collection && (
              <View className='mb-3 flex-row items-center gap-2'>
                ...
                />
                <Text className='flex-1 text-sm text-light-textSecondary dark:text-dark-textSecondary'>
                  {t('movieDetail.partOf') || 'Part of'}{' '}
                  {detailedInfo.belongs_to_collection.name}
                </Text>
              </View>
            )}
            {mediaType === 'tv' &&
              detailedInfo.networks &&
              detailedInfo.networks.length > 0 && (
                <View className='mb-3 flex-row items-center gap-2'>
                  <Ionicons
                    name='tv-outline'
                    size={15}
                  ...
                  <Text className='flex-1 text-sm text-light-textSecondary dark:text-dark-textSecondary'>
                    ...
                    {detailedInfo.networks.map(n => n.name).join(', ')}
                  </Text>
                </View>
              )}
            {mediaType === 'tv' && detailedInfo.in_production && (
      ...
      {detailedInfo.production_companies &&
        detailedInfo.production_companies.length > 0 && (
          <MotiView
            from={{ opacity: 0, translateY: 15 }}
            animate={{ opacity: 1, translateY: 0 }}
                ...
                {detailedInfo.production_companies.map(company => (
                  ...
                  >
                    {company.logo_path ? (
                      <Image
                        source={{
                          uri: `https://image.tmdb.org/t/p/w92${company.logo_path}`,
                        }}
        ...
        <View className='mb-8 flex-row gap-3'>
          {detailedInfo.imdb_id && (
            <TouchableOpacity
              onPress={() =>
                Linking.openURL(
                  `https://www.imdb.com/title/${detailedInfo.imdb_id}`
                )
              }
              activeOpacity={0.7}
