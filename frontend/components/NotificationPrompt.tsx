...
export default function NotificationPrompt({ searchCount }: NotificationPromptProps) {
  ...
  const handleEnable = async () => {
    ...
    if (token) {
      ...
      try {
        await backendAPIService.registerPushToken(token, Platform.OS as 'ios' | 'android');
      } catch (error) {
        Sentry.captureException(error, { tags: { context: 'push-token-registration' } });
      }

      // Schedule daily pick reminder
      const dailyTitle = t('notifications.dailyTitle') || 'Your daily pick is ready!';
      const dailyBody =
        t('notifications.dailyBody') || "Check out today's personalized recommendation";
    ...
    <Modal visible={visible} transparent animationType="fade" statusBarTranslucent>
      <View className="flex-1 items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
        <MotiView
          ...
          style={[getSquircle(24), { width: '85%', maxWidth: 380, overflow: 'hidden' }]}
        >
          <BlurView
          ...
          >
            ...
            >
              ...
              {/* Title */}
              <Text
                className="mb-2 text-center text-xl font-bold text-light-text dark:text-dark-text"
              >
              ...
              {/* Subtitle */}
              <Text
                className="mb-6 text-center text-sm leading-5 text-light-muted dark:text-dark-muted"
              >
              ...
              <View className="mb-6 gap-3">
                {features.map((feature, index) => (
                  <MotiView
                    ...
                    transition={{ delay: 100 + index * 80, type: 'timing', duration: 300 }}
