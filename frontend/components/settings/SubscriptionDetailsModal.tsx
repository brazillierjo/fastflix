    ...
    >
        <SafeAreaView className="flex-1 bg-light-background dark:bg-dark-background">
          ...
          <View className="flex-1 px-4 pt-6">
            {/* Status Card */}
            <View className="rounded-xl bg-light-card dark:bg-dark-card p-4 mb-6">
              <View className="flex-row items-center mb-3">
                <View
                  className={`h-10 w-10 rounded-full items-center justify-center mr-3 ${
                    isSubscribed ? 'bg-green-500/20' : 'bg-light-muted/20 dark:bg-dark-muted/20'
                  }`}
                >
                  <Ionicons
                    name={isSubscribed ? 'checkmark-circle' : 'close-circle'}
                    size={24}
                    color={isSubscribed ? '#22c55e' : (isDark ? '#6b7280' : '#9ca3af')}
                  />
                </View>
                <View className="flex-1">
                  <Text className="text-lg font-semibold text-light-text dark:text-dark-text">
                    {isSubscribed
                      ...
                      : t('subscription.required.title') || 'No Active Subscription'}
                  </Text>
                  {isSubscribed && (
                    <Text className="text-sm text-light-muted dark:text-dark-muted">
                      {t('profile.enjoyPremiumFeatures') || 'Enjoy all FastFlix Pro features'}
                    </Text>
                  )}
                </View>
              </View>
              ...
              {/* Trial Info */}
              {isInTrial && trialInfo && (
                <View className="rounded-lg bg-amber-500/10 dark:bg-amber-500/20 p-3 mt-2">
            ...
            {/* Info Text */}
            {!isSubscribed && (
              <Text className="mt-4 text-center text-sm text-light-muted dark:text-dark-muted px-4">
